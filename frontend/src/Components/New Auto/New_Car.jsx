...
import s from "./New_Car.module.css"
...
const New_Car = () => {
  const { cars, setPage, page, setRowsPerPage, rowsPerPage, totalItems, getNewCars } = useGlobal();
        ...
        {cars.map(car => (
            ...
            <table className={s.width} key={car.id}>
              <tr>
                ...
                <td>
                  <h3>
                    <NavLink to={`/cars/${car.id}`} onClick={() => carClickHandler(car.id)}>{car.car_title}</NavLink>
                  </h3>
                </td>
                <td>
                  <center>
